...
class SemanticViewer(QWidget):
    ...
    def compute_ground_clearance(self, curve_points, catenary_params=None):
        ...
        Uses analytical catenary minimum if params available, otherwise finds
        the point with minimum Z value.
        
        Args:
            curve_points: numpy array of curve points (N, 3)
            catenary_params: tuple (a, b, c, t0, mean_xy, u_xy, mean_perp, perp_xy) or None
        """
        ...
        # Method 1: Analytical solution using catenary parameters (BEST)
        if catenary_params is not None:
            try:
                a, b, c, t0, mean_xy, u_xy, mean_perp, perp_xy = catenary_params
                
                # For catenary z(t) = a * cosh((t - b) / a) + c
                # The minimum occurs at t = b (where derivative = 0)
                # This is the actual sag point!
                
                t_min_norm = b  # minimum occurs at t = b
                t_min = t_min_norm + t0  # convert back to original t coordinate
                
                # Compute 3D point at the sag
                xy_min = mean_xy + t_min * u_xy + mean_perp * perp_xy
                z_min = a * np.cosh((t_min_norm - b) / (a + 1e-12)) + c
                
                lowest = np.array([xy_min[0], xy_min[1], z_min])
                
                print(f"Using analytical sag point: z={z_min:.3f}m")
                
            except Exception as e:
                print(f"Error computing analytical minimum: {e}, falling back to argmin")
                # Fallback to Method 2
                idx_low = int(np.argmin(curve_points[:, 2]))
                lowest = curve_points[idx_low]
        else:
            # Method 2: Find minimum Z in curve points (FALLBACK)
            idx_low = int(np.argmin(curve_points[:, 2]))
            lowest = curve_points[idx_low]
            print(f"Using minimum Z point (no catenary params)")
