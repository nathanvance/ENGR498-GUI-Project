...
def build(force: bool = False) -> Path:
    ...
    source = native_dir / "pointcloud_accel.cpp"
    build_dir = native_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    dll_path = build_dir / "pointcloud_accel.dll"
    obj_path = build_dir / "pointcloud_accel.obj"
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Failed to build pointcloud_accel.dll\n"
            f"stdout:\n{result.stdout}\n"
            f"stderr:\n{result.stderr}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the Fusion point-cloud accelerator DLL.")
