...
def build(force: bool = False) -> Path:
    ...
    source = native_dir / "geospatial_accel.cpp"
    build_dir = native_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    dll_path = build_dir / "geospatial_accel.dll"
    obj_path = build_dir / "geospatial_accel.obj"

    if (
        not force
        and dll_path.exists()
        and dll_path.stat().st_mtime >= source.stat().st_mtime
    ):
        return dll_path
    ...
    batch_path = build_dir / "build_geospatial_native.bat"
    batch_path.write_text(
        "@echo off\n"
        f'call "{vsdevcmd}" -arch=amd64\n'
        "if errorlevel 1 exit /b %errorlevel%\n"
        f'cl /nologo /LD /O2 /EHsc /std:c++17 /openmp /Fo"{obj_path}" "{source}" /link /OUT:"{dll_path}"\n',
        encoding="utf-8",
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Failed to build geospatial_accel.dll\n"
            f"stdout:\n{result.stdout}\n"
            f"stderr:\n{result.stderr}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the Fusion geospatial accelerator DLL.")
    parser.add_argument("--force", action="store_true", help="Rebuild even if the DLL is up to date.")
    args = parser.parse_args()

    dll_path = build(force=args.force)
    print(dll_path)
    return 0
