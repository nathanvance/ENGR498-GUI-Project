#!/usr/bin/env python3
"""
LAS Normal Estimation Tool

Estimates normal vectors for LAS point cloud files and saves them as extra dimensions.
Works with filtered or unfiltered LAS files.
...
def select_output_file(default_name):
    ...
    file_path = filedialog.asksaveasfilename(
        title="Save LAS file with normals as",
...
def estimate_normals_for_las(input_las_path, output_las_path, search_radius=0.1, max_nn=30):
    """
    Estimate normals for a LAS file and save with normals as extra dimensions.
    
    Args:
        input_las_path: Path to input LAS file
        output_las_path: Path to output LAS file with normals
    ...
    # Create output LAS file with same format as input
    print(f"\nCreating output LAS file: {output_las_path}")
    
    # Check if input already has normal extra dimensions
    has_existing_normals = False
    try:
        _ = las_in.normal_x
        has_existing_normals = True
        print("  Input file already has normal dimensions - will overwrite")
    except:
        pass
    
    # Create new LAS file with same header but add normal dimensions if needed
    header = laspy.LasHeader(point_format=las_in.point_format, version=las_in.header.version)
    header.offsets = las_in.header.offsets
    header.scales = las_in.header.scales
    
    # Add extra dimensions for normals if they don't exist
    if not has_existing_normals:
    ...
    # Copy all point data from input
    las_out.x = las_in.x
    las_out.y = las_in.y
    las_out.z = las_in.z
    ...
    if hasattr(las_in, 'classification'):
        ...
        print("  Copied classification values")
    
    # Copy any other standard fields
    for dimension in las_in.point_format.dimension_names:
        if dimension not in ['X', 'Y', 'Z', 'intensity', 'red', 'green', 'blue', 'classification']:
            try:
                setattr(las_out, dimension.lower(), getattr(las_in, dimension.lower()))
            except:
                pass
    
    # Add the estimated normals
    las_out.normal_x = normals[:, 0]
    las_out.normal_y = normals[:, 1]
    las_out.normal_z = normals[:, 2]
    print("✓ Normal vectors written to LAS file")
    ...
    try:
        ...
        print(f"Normal vectors added as extra dimensions:")
        print(f"  - normal_x")
        print(f"  - normal_y")
        print(f"  - normal_z")
        print(f"{'='*60}")
...
def main():
    ...
    print("=" * 60)
    print("LAS Normal Estimation Tool")
    print("=" * 60)
    print()
    ...
    default_output = f"{base_name}_with_normals.las"
    
    # Select output file
    print("Please select where to save the LAS file with normals...")
    ...
    success = estimate_normals_for_las(input_path, output_path, search_radius=search_radius)
    ...
    if success:
        messagebox.showinfo(
            ...
            f"Output file:\n{output_path}\n\n"
            f"Normal vectors stored as extra dimensions:\n"
            f"- normal_x\n"
            f"- normal_y\n"
            f"- normal_z"
        )
